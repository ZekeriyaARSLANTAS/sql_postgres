...
select state,count(state)as nuber_of_employees
from workers
group by state
...
select distinct name from workers; --distinct çagrılan terimlerden sadece birincisini alır
...
--concat() fonsiyonu bir den fazla sutun veya string i birlestirir
--1 yol
...
from workers
order by length(name)+length (state) desc
/*
-- UNION oparator :1) iki sorgu (query) sonucunu birleştirmek için kullanılır.
...
intersect -- ortak olanları tekrarsız getirir
...
EXCEPT ---çıkarmak için kullanılır
...
select employee.name as Employee,maneger.name as maneger
from workers employee full JOIN workers maneger
on employee.manager_id=maneger.id
